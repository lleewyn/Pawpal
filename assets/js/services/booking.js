...
// ── "2-hour ahead" validation ─────────────────────────────────────────────────
function isSlotTooSoon(dateStr, slotStr) {
    if (!dateStr || !slotStr) return false;
    const [hour, minute] = slotStr.split(':').map(Number);
    const slotTime = new Date(`${dateStr}T${slotStr}:00`);
    const minAllowed = new Date(Date.now() + 2 * 60 * 60 * 1000); // now + 2h
...
function renderTimeSlots() {
    ...
    grid.innerHTML = ALL_SLOTS.map(slot => {
        ...
        const tooSoon = isSlotTooSoon(state.schedule.date, slot);
        ...
        const disabled = busy || heldByOther || tooSoon;

        let label = slot;
        let subLabel = '';
        if (busy) subLabel = '<br><small>Đầy</small>';
        else if (heldByOther) subLabel = '<br><small>Đang chờ</small>';
        else if (tooSoon) subLabel = '<br><small style="color:#ef4444">Quá gần</small>';

        return `<button class="timeslot-btn ${selected ? 'selected' : ''} ${heldByOther ? 'held' : ''} ${tooSoon ? 'too-soon' : ''}"
                    data-slot="${slot}" ${disabled ? 'disabled' : ''}
                    title="${tooSoon ? 'Vui lòng đặt trước ít nhất 2 tiếng' : ''}"
                    aria-label="${slot}${busy ? ' (đã đầy)' : heldByOther ? ' (Đang chờ)' : tooSoon ? ' (Quá gần giờ hiện tại)' : ''}">
                    ${label}${subLabel}
                </button>`;
    }).join('');
...
function checkStep3Valid() {
    ...
    let valid = false;
    if (svc?.isHotel) {
        // Hotel: check-in phải ít nhất ngày mai (2h rule không áp dụng cho hotel check-in cả ngày)
        const checkInDate = state.schedule.checkIn ? new Date(state.schedule.checkIn + 'T00:00:00') : null;
        const minCheckIn = new Date(Date.now() + 2 * 60 * 60 * 1000);
        const checkInTooSoon = checkInDate && checkInDate < minCheckIn;
        ...
        const tooSoon = isSlotTooSoon(state.schedule.date, state.schedule.slot);
        valid = !!(state.schedule.date && state.schedule.slot && state.selectedStaffId &&
                   !isSlotHeldByOther() && !tooSoon);
    }
...
// Auto re-render timeslots mỗi phút để "Quá gần" cập nhật theo thời gian thực
setInterval(() => {
    if (state.currentStep === 3 && state.schedule.date) {
        renderTimeSlots();
        checkStep3Valid();
    }
}, 60 * 1000);
